...
# ── The plan engine lives in sh_engine.py (TECH-12) ─────────────────────────
# Everything the PLAN is computed from left this file: the engine proper, the §FT
# feasibility model, the CTL/ATL projector, the pace/VDOT maths, and the handful of
# store/settings readers those need. The arrow is one-way — sh_engine imports nothing
# from here — so this file is the web/IO layer and that one is the deterministic core.
...
from sh_engine import (   # noqa: F401 — re-exported for the app and the battery
    ...
    FT10_DISP_H, FT10_DISP_K, FT10_DISP_MIN_W, FT10_DISP_P, FT2_CEIL_HEADROOM, FT2_EWMA_A, FT2_R,
    ...
    find_duplicates, fmt_pace, generate_block, generate_plan, generate_rebase, get_meta,
    latest_snapshot, manual_ignores, pace_zones, peak_shape, periodize, periodize_chain,
    plan_baseline, plan_seed, project_forward, reconstruct_history, roll, score_finish,
    select_chain, set_meta, shape_response, taper_shape, training_regime, weeks_until)
...
# Reading a setting is engine work — the plan needs the manual LTHR and the date of birth — so
# SETTINGS_SPEC and `_resolve_setting` went with the engine (TECH-12). What stays here is the part
# that is about the console: rendering the settable set with its provenance, validating what a human
# typed, and applying a save to the live process.
